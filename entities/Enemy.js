...
export const ENEMY_TYPE_DEFS = Object.fromEntries(
  getAllEnemyDefinitions().map((definition) => [definition.id, {
    ...
    collisionGroup: definition.collisionGroup ?? 'enemy',
    ignoreCollisionWith: Array.isArray(definition.ignoreCollisionWith) ? [...definition.ignoreCollisionWith] : [],
...
export class Enemy extends Entity {
  constructor(type, x, y) {
    ...
    super({
      ...
      collisionGroup: def.collisionGroup ?? 'enemy',
      ignoreCollisionWith: Array.isArray(def.ignoreCollisionWith) ? [...def.ignoreCollisionWith] : [],
      frozen: false,
      freezeTint: null,
      freezeGlow: null,
      hitFlashTimer: 0,
      ...
      attackRate: def.attackRate ?? (1 + Math.random() * 0.5),
