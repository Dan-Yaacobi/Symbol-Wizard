...
export class SpriteEditorScreen {
  ...
  #paintAt(event) {
    ...
    const style = getComputedStyle(this.elements.canvas);
    ...
    const scaleX = this.elements.canvas.width / contentWidth;
    const scaleY = this.elements.canvas.height / contentHeight;
    const x = Math.floor((((event.clientX - rect.left - borderLeft) * scaleX) - 16) / 24);
    const y = Math.floor((((event.clientY - rect.top - borderTop) * scaleY) - 16) / 24);
