...
function resolveEntityAnimationState(entity, moving) {
  if (entity.isAttacking || (entity.castTimer ?? 0) > 0) return 'attack';
...
export function updateEntityAnimation(entity, dt, moving, config = null) {
  ...
  const nextState = resolveEntityAnimationState(entity, moving);
  ...
  if (!frames?.length) {
    throw new Error(`Missing sprite animation "${entity.animationState}" for sprite "${entity.spriteId}".`);
  }
