...
function entranceSpawnCacheKey(entrance, preferredX, preferredY) {
...
function collectExitTriggerTiles(room) {
  ...
  for (const exit of exits) {
    ...
    if (Number.isFinite(px) && Number.isFinite(py)) triggerTiles.add(`${px},${py}`);
  }
  for (const corridor of corridors) {
    for (const tile of corridor?.triggerTiles ?? []) {
      const tx = Math.round(tile?.x ?? Number.NaN);
      const ty = Math.round(tile?.y ?? Number.NaN);
      if (Number.isFinite(tx) && Number.isFinite(ty)) triggerTiles.add(`${tx},${ty}`);
    }
  }
...
function getRoomTransitionCache(room) {
  if (!room) {
    return {
      ...
      exitTriggerTiles: new Set(),
      walkableCandidates: [],
      spawnByEntrance: new Map(),
    };
  }
  ...
  for (const object of room?.objects ?? []) {
    ...
    for (const [dx, dy] of footprint) {
      blockedTiles.add(`${Math.round(object.x + dx)},${Math.round(object.y + dy)}`);
    }
  }
  ...
  const nextCache = {
    ...
    exitTriggerTiles,
    walkableCandidates,
    spawnByEntrance: cache?.spawnByEntrance ?? new Map(),
  };
  room.__transitionCache = nextCache;
  return nextCache;
}

function findSpawnPosition(room, preferredX, preferredY, options = {}) {
  const profile = options.profile ?? { attempts: 0, checks: 0, retries: 0, sections: {} };
  const markSection = (key, startMs) => {
    const duration = nowMs() - startMs;
    profile.sections[key] = Number(((profile.sections[key] ?? 0) + duration).toFixed(3));
  };

  const startX = Math.round(preferredX);
  const startY = Math.round(preferredY);
  const roomCache = getRoomTransitionCache(room);
  const key = `${startX},${startY}`;

  const candidateSelectionStart = nowMs();
  profile.attempts += 1;
  markSection('candidate_selection', candidateSelectionStart);

  const validationStart = nowMs();
  profile.checks += 1;
  if (isWalkableTile(room, startX, startY) && !roomCache.blockedTiles.has(key) && !roomCache.exitTriggerTiles.has(key)) {
    markSection('validation_checks', validationStart);
    return { x: startX, y: startY };
  }
  markSection('validation_checks', validationStart);

  const ringSearchStart = nowMs();
  ...
  for (let radius = 1; radius <= maxRadius; radius += 1) {
    ...
    for (let x = left; x <= right; x += 1) {
      const topKey = `${x},${top}`;
      profile.checks += 1;
      if (isWalkableTile(room, x, top) && !roomCache.blockedTiles.has(topKey) && !roomCache.exitTriggerTiles.has(topKey)) {
        markSection('loop_search', ringSearchStart);
        return { x, y: top };
      }
      if (bottom === top) continue;
      const bottomKey = `${x},${bottom}`;
      profile.checks += 1;
      if (isWalkableTile(room, x, bottom) && !roomCache.blockedTiles.has(bottomKey) && !roomCache.exitTriggerTiles.has(bottomKey)) {
        markSection('loop_search', ringSearchStart);
        return { x, y: bottom };
      }
    }
    for (let y = top + 1; y < bottom; y += 1) {
      const leftKey = `${left},${y}`;
      profile.checks += 1;
      if (isWalkableTile(room, left, y) && !roomCache.blockedTiles.has(leftKey) && !roomCache.exitTriggerTiles.has(leftKey)) {
        markSection('loop_search', ringSearchStart);
        return { x: left, y };
      }
      if (right === left) continue;
      const rightKey = `${right},${y}`;
      profile.checks += 1;
      if (isWalkableTile(room, right, y) && !roomCache.blockedTiles.has(rightKey) && !roomCache.exitTriggerTiles.has(rightKey)) {
        markSection('loop_search', ringSearchStart);
        return { x: right, y };
      }
    }
  ...
  for (const candidate of roomCache.walkableCandidates) {
    profile.checks += 1;
  ...
  markSection('fallback_search', fallbackStart);
...
export class RoomTransitionSystem {
  ...
  switchRoom(context, exitRef) {
    ...
    const cachedSpawn = cache.spawnByEntrance.get(spawnKey);
    const spawnProfile = {
      attempts: 0,
      checks: 0,
      retries: 0,
      sections: {},
      usedCache: Boolean(cachedSpawn),
    };
    const spawn = cachedSpawn ?? findSpawnPosition(targetRoom, preferredSpawn.x, preferredSpawn.y, { profile: spawnProfile });
    if (!cachedSpawn) {
      cache.spawnByEntrance.set(spawnKey, { x: spawn.x, y: spawn.y });
    }
    switchProfile.spawnSearch = spawnProfile;
    ...
    console.info('[TransitionTimingDetailed]', {
      ...
      spawnSearch: switchProfile.spawnSearch,
    });
